...
private:
    std::array<coordinate_type, dimensions> coords_;	//배열, coordinate_type으로 dimensions 크기의 배열을 생성
};
...
public:
	...
	std::vector<point_type> getPoints()
	{
		std::vector<point_type> p_;
		for(int i=0; i<nodes_.size(); i++)
		{
			p_.push_back(nodes_[i].point_);
		}
...
void test_random(size_t count) {
	...
	std::vector<point3d> points = tree.getPoints();
 
    std::cout << "Random data (" << count << ", " << tree.size() << " points):\n";
    std::cout << "point: " << pt << '\n';
    std::cout << "nearest point: " << n << '\n';
    std::cout << "distance: " << tree.distance() << '\n';
    std::cout << "nodes visited: " << tree.visited() << '\n';
	...
	std::cout << "[0] : COOR : " << points[0].get(0) << points[0].get(1) << points[0].get(2) << std::endl;
	// for(int i=0; i<points.size(); i++)
	// {
	// 	printf("(%f, %f, %f) ", points[i].get)
	// }
}
